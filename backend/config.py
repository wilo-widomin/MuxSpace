...
AUTH_ENABLED: bool = _get_bool("TMUX_PANEL_AUTH_ENABLED", True)

# Cómo se validan las credenciales del login:
#   env -> usuario/contraseña fijados abajo (TMUX_PANEL_USERNAME/PASSWORD).
...
AUTH_MODE: str = os.getenv("TMUX_PANEL_AUTH_MODE", "env").strip().lower()
if AUTH_MODE not in {"env", "pam"}:
    raise ValueError(
        f"TMUX_PANEL_AUTH_MODE inválido: {AUTH_MODE!r} (usa 'env' o 'pam')."
    )
...
PAM_SERVICE: str = os.getenv("TMUX_PANEL_PAM_SERVICE", "login")

AUTH_USERNAME: str = os.getenv("TMUX_PANEL_USERNAME", "admin")
AUTH_PASSWORD: str = os.getenv("TMUX_PANEL_PASSWORD", "admin")

# Horas de validez de la sesión iniciada en /api/login (cookie HttpOnly).
SESSION_TTL_HOURS: int = int(os.getenv("TMUX_PANEL_SESSION_TTL_HOURS", "168"))

# Marca la cookie de sesión como `Secure` (solo viaja por HTTPS). Actívalo
# cuando sirvas el panel tras un reverse proxy con TLS.
COOKIE_SECURE: bool = _get_bool("TMUX_PANEL_COOKIE_SECURE", False)
...
HOST: str = os.getenv("TMUX_PANEL_HOST", "127.0.0.1")
PORT: int = int(os.getenv("TMUX_PANEL_PORT", "8000"))

# Binario de tmux (por si no está en el PATH estándar).
TMUX_BINARY: str = os.getenv("TMUX_PANEL_TMUX_BINARY", "tmux")
...
DIR_SUGGESTION_ROOTS: list[str] = _get_str_list(
    "TMUX_PANEL_DIR_SUGGESTION_ROOTS", ["~"]
)
